...
def handle_url(links, posts, users, url, img_root_dir, txt_root_dir):
    if ".backbook.me/" in url:
        if url not in links:
            links[url] = extract_backbook(url)
        item = links[url]
        img_dir = create_sub_dir(img_root_dir, 'backbook')
        download_backbook(item, img_dir, overwrite_time=None)
        return
    ...
    if ".imagevenue.com/" in url:
        ...
        download_imagevenue(item, img_dir, overwrite_time=None)
        return
    ...
    if ".sxnarod.com/" in url:
        ...
        download_sxnarod(posts, item, img_dir, txt_dir, users, links)
        return
